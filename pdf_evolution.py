...
for model, model_name in zip(model_folders, models):
    ...
    for dim_run in os.listdir(trained_folder):
        ...
        for layer in pdf_dict:
            ...
            writer = anime.FFMpegWriter(fps=y.shape[0]//45, metadata=meta)

            fig, ax = plt.subplots(figsize=(10,7))

            lines, = ax.plot(x, y[0], '-k')
            ax.set(xlim=x[[0,-1]], ylim=(0, y.max()))
            ax.set_xlabel("Weight", size=17.5)
            ax.set_ylabel("Density", size=17.5)
            ax.tick_params(which='both', length=9, labelsize=12.5)
            ax.set_title(f"Density evolution of {layer.replace('_', ' ').capitalize()}, Latent Dim: {lat_dim},\n"
                         + f"Inter epoch {0:03d}", 
                         size=25)
            ...
            with writer.saving(fig, os.path.join(model_weight_folder, f"pdf_{layer}.gif"), 150):
                for ii in range(y.shape[0]):
                    ax.set_title(f"Density evolution of {layer.replace('_', ' ').capitalize()}, Latent Dim: {lat_dim},\n"
                                + f"Inter epoch {ii:03d}", 
                                size=25)
