...
def train_model(hyp_params, train_data, val_set, model, loss):
    ...
    if 'train_loss_results' not in hyp_params:
        hyp_params['train_loss_results'] = []
    if 'val_loss_results' not in hyp_params:
        hyp_params['val_loss_results'] = []
    if 'last_epoch' not in hyp_params:
        hyp_params['last_epoch'] = 0

    if not os.path.exists(hyp_params['model_path']):
        os.makedirs(hyp_params['model_path'])
    if not os.path.exists(os.path.join(hyp_params['model_path'], 'params')):
    ...
    for epoch in range(hyp_params['last_epoch'] + 1, hyp_params['max_epochs'] + 1):
        ...
        hyp_params['train_loss_results'].append(np.log10(epoch_loss_avg_train.result()))
        hyp_params['val_loss_results'].append(np.log10(epoch_loss_avg_val.result()))
        ...
        if epoch == 1 or epoch % hyp_params['plot_every'] == 0:
            ...
            this_plot = os.path.join(hyp_params['plot_path'], f"{epoch_start_time.strftime(r'%Y%m%d%H%M%S')}.png")
        ...
        if epoch % hyp_params['save_every'] == 0 or epoch == hyp_params['max_epochs']:
            pass
            #model_path = os.path.join(hyp_params['model_path'], 'params', f"epoch_{epoch:05d}_loss_{train_params['val_loss_results'][-1]:2.3}")
            #model.save_weights(model_path + '.h5')
            #pickle.dump(hyp_params, open(model_path + '.pkl', 'wb'))

        
        model_path = os.path.join(hyp_params['model_path'], f'epoch_{epoch:05d}')
        model.save_weights(model_path + '.h5')
        pickle.dump(hyp_params, open(os.path.join(hyp_params['model_path'], "params", "last_epoch.pkl"), "wb"))
